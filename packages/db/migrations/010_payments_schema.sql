-- Migration: Payments schema with RLS policies
...
CREATE TABLE payments.payments (
    ...
    provider_payment_id TEXT, -- Stripe payment_intent or session id
    amount_cents INTEGER NOT NULL CHECK (amount_cents > 0),
    currency TEXT NOT NULL DEFAULT 'usd',
    received_at TIMESTAMPTZ, -- set on success
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'succeeded', 'failed')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE payments.allocations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL REFERENCES hr.organizations(id),
